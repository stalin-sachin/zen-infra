...
resource "aws_iam_role_policy" "runner" {
  ...
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      # ECR auth (account-wide, no resource restriction possible)
      {
        Sid      = "ECRAuth"
        Effect   = "Allow"
      ...
      # ECR push to all repos in this account
      {
        Sid    = "ECRPush"
        Effect = "Allow"
        Action = [
      ...
      # EKS read (needed by deploy steps that call eks:DescribeCluster)
      {
        Sid      = "EKSRead"
        Effect   = "Allow"
      ...
      # Secrets Manager — read the GitHub PAT for runner registration token generation
      {
      ...
      # SSM Session Manager — SSH-less instance access via AWS console/CLI
      {
...
# ── Security Group — egress only (runner dials GitHub, GitHub never dials in) ─
resource "aws_security_group" "runner" {
  ...
  egress {
    description = "All outbound (GitHub, ECR, SSM endpoints)"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
